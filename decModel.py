...
class DECModel(model.MXModel):
    ...
    def cluster(self, X, y, classes, save_to, update_interval=None):
        N = X.shape[0]
        if not update_interval:
            update_interval = int(self.batch_size)
        ...
        pp = 10
        tsne = TSNE(n_components=2, perplexity=pp, learning_rate=200,
        ...
        fig.savefig(save_to+os.sep+'tsne_init_k'+str(self.num_centers)+'_z'+str(self.znum)+'.pdf', bbox_inches='tight')    
        plt.close()  
        ...
        solver = Solver('sgd', momentum=0.9, wd=0.0, learning_rate=0.1)
        def ce(label, pred):
        ...
        self.plot_interval = int(4*N/float(self.batch_size))
        ...
        def refresh(i):
            if i%update_interval == 0:
                ...
                # tol% = 0.001
                if i == self.batch_size*200: # performs 1epoch = 615/3 = 205*1000epochs                     
                    self.y_pred = y_pred
                    return True 
        ...
        # finish                
        fig = plt.gcf()
        fig.savefig(save_to+os.sep+'tsne_progress_k'+str(self.num_centers)+'_z'+str(self.znum)+'.pdf', bbox_inches='tight')    
        plt.close()    
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename=os.path.join(save_to,'decModel.log'), format='%(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p',level=logging.DEBUG)                    
    ...
    Y = YnxG_allNME[2]+['_'+str(yl) for yl in YnxG_allNME[3]] 
    ...
    for znum in latent_size:
        ...
        dec_model = DECModel(mx.cpu(), combX_allNME, num_centers, 1.0, znum, save_to)
        outdict = dec_model.cluster(combX_allNME, y_dec, classes, save_to, update_interval=12) # ~ 798/32=24 ~ N/(batch size)=iterations to reach a full epochg
        # save output results
    ...
    # save output
    allDEC_file = gzip.open(os.path.join(save_to,'allDEC_log.pklz'), 'wb')
    ...
    axDEC_perf = sns.pointplot(x="iterations", y="acc", hue="Z-spaceDim", data=dfDEC_perf)                               
