...
class DECModel(model.MXModel):
    ...
    def setup(self, X, num_centers, alpha, znum, save_to='dec_model'):
        ...
        batch_size = 80 # 266 #X.shape[0]
        ae_model = AutoEncoderModel(self.xpu, [X.shape[1],500,500,2000,znum], pt_dropout=0.2)
        ae_model.load( os.path.join(save_to,'SAE_zsize{}_wimgfeatures.arg'.format(str(znum))) )
    ...
    def cluster_unsuperv(self, X, y_dec, roi_labels, classes, save_to, labeltype, update_interval=None):
        ...
        def refresh(i): # all epoch occurs every i=798/133 = 6 iteration for an epochs, update every 1 epochos
...
if __name__ == '__main__':
    ...
    ######################
    labeltype = 'NME_dist_int_enh_wimgfeature' # roilabel_NME_dist
    ...
    for znum in latent_size:
        ...
        print "Loading autoencoder of latent size znum = ",znum
        save_to = r'Z:\Cristina\Section3\NME_DEC\SAEmodels'
        ...
        outdict = dec_model.cluster_unsuperv(X, y_dec, roi_labels, classes, save_to, labeltype, update_interval=12) # ~ 798/48=16 update twice per epoch ~ N/(batch size)=iterations to reach a full epochg
        ...
        dec_model_save= gzip.open(os.path.join(save_to,'dec_model_znum{}_wimgfeatures_y{}.arg'.format(znum,labeltype)), 'wb')
        ...
        # plot current iter
        df = pd.DataFrame({'acc': pd.Series(outdict['acc']), 'iterations':range(len(outdict['acc'])), 'Z-spaceDim':outdict['z'].shape[1] })
        fig = plt.figure(figsize=(20,6))
        ax = plt.axes()
        sns.set_context("notebook")
        sns.set_style("darkgrid", {"axes.facecolor": ".9"})    
        sns.pointplot(x="iterations", y="acc", hue="Z-spaceDim", data=df, ax=ax, 
                                   markers=[".",".",".","."], linestyles=["-","-","-","-","-"], size=0.05) 
        fig.autofmt_xdate(bottom=0.2, rotation=30, ha='right')   
        
    
    # save output
    allDEC_file = gzip.open(os.path.join(save_to,'allDEC_wimgfeatures_y{}.pklz'.format(labeltype)), 'wb')
    ...
    sns.pointplot(x="iterations", y="acc", hue="Z-spaceDim", data=dfDEC_perf, ax=ax, 
                               markers=[".",".",".","."], linestyles=["-","-","-","-","-"], size=0.05) 
    fig.autofmt_xdate(bottom=0.2, rotation=30, ha='right')   
    fig.savefig(save_to+os.sep+'allDEC_wimgfeatures_y{} Dataset_unsuprv acc vs iteration.pdf'.format(labeltype), bbox_inches='tight')    
