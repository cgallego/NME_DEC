# -*- coding: utf-8 -*-
"""
Created on Fri Dec 01 13:31:43 2017
...
class DECModel(model.MXModel):
    ...
    def cluster(self, X_train, y_dec_train, y_train, classes, batch_size, save_to, labeltype, update_interval, logger):
        ...
        def ce(label, pred):
            ...
            cv = StratifiedKFold(n_splits=2, random_state=3)
            ...
            Z_train_iter = mx.io.NDArrayIter(self.Z_train, self.yZ_train, batch_size)
            self.mlp_model.fit(Z_train_iter,  # train data
                              monitor=None,
                              optimizer='sgd',  # use SGD to train
                              optimizer_params={'learning_rate':0.1},  # use fixed learning rate
                              eval_metric= 'acc', #MLPacc(yZ_val, Z_val),  # report accuracy during trainin
                              num_epoch=100)
            prob_train = self.mlp_model.predict(Z_train_iter)
            p_y_given_x = prob_train.asnumpy()
            ...
            # Learning optimal model parameters involves minimizing a loss function. In the case of multi-class logistic regression, it is very common to use the negative log-likelihood as the loss. This is equivalent to maximizing the likelihood of the data set \cal{D} under the model parameterized by \theta.
            ...
            #NLL = -np.mean( np.concatenate((np.log(p_y_given_x), self.yZ_train.reshape(self.yZ_train.shape[0],-1)), axis=1))
            NLL = log_loss(self.yZ_train, p_y_given_x, eps=1e-15)

            overall_metric = DECmetric + NLL 
        ...
        def refresh(i): # i=3, a full epoch occurs every i=798/48
            if i%self.best_args['update_interval'] == 0:
                ...
                sep = int(792*0.10) # to keep same ind test set
                print(self.z.shape)
                y = np.asarray(self.best_args['roi_labels'])
                Z = np.concatenate((self.z, self.p), axis=1) #zbestacci #dec_model['zbestacci']   
                print(Z)
...
if __name__ == '__main__':
    ##################################################### 
    from decModel_wimgF_dualopt_descStats_onlyLabels import *
    from utilities import *
    ...
    ######################            
    # select only Labeled cases     
    indL = nxGdatafeatures['classNME']!='U'
    nxGdatafeatures_onlyL = nxGdatafeatures[indL]
    
    YnxG_allNME = np.asarray([nxGdatafeatures_onlyL['roi_id'].values,
            nxGdatafeatures_onlyL['classNME'].values,
            nxGdatafeatures_onlyL['nme_dist'].values,
            nxGdatafeatures_onlyL['nme_int'].values])
    # shape input (792L, 523L)   
    ...
    # shape input selecting only Labeled cases  = (377L, 523L)
    combX_allNME = combX_allNME[indL,:]
    ...
    # set for training
    labeltype = 'wimgF_dualopt_descStats_onlyLabels' 
    save_to = r'Z:\Cristina\Section3\NME_DEC\SAEmodels\decModel_wimgF_dualopt_descStats_onlyLabels'
