...
class DECModel(model.MXModel):
    ...
    def cluster_varying_mu(self, X, y_dec, roi_labels, classes, save_to, labeltype, update_interval=None):
        ...
        fig.savefig(save_to+os.sep+'iter1_tsne_init_z'+str(self.best_args['znum'])+'_varying_mu'+str(self.best_args['num_centers'])+'.pdf', bbox_inches='tight')    
        plt.close()  
        ...
        solver = Solver('sgd', momentum=0.9, wd=0.0, learning_rate=0.01, lr_scheduler=mx.misc.FactorScheduler(100*update_interval,0.5)) #0.01
        def ce(label, pred):
        ...
        def refresh(i): # i=3, a full epoch occurs every i=798/48
            if i%self.best_args['update_interval'] == 0:
                ...
                # tol% = 0.001
                if i == self.best_args['update_interval']*600: # performs 1epoch = 615/3 = 205*1000epochs                     
        ...
        # finish                
        fig = plt.gcf()
        fig.savefig(save_to+os.sep+'iter1_tsne_progress_z'+str(self.best_args['znum'])+'_varying_mu'+str(self.best_args['num_centers'])+'.pdf', bbox_inches='tight')    
        plt.close()    
        ...
        fig.savefig(save_to+os.sep+'tsne_final_z'+str(self.best_args['znum'])+'_varying_mu'+str(self.best_args['num_centers'])+'.pdf', bbox_inches='tight')    
        plt.close()  
...
if __name__ == '__main__':
    ...
    ######################
    labeltype = 'varying_mu' # roilabel_NME_dist
    save_to = r'Z:\Cristina\Section3\NME_DEC\SAEmodels\decModel_exp3'
    ...
    for num_centers in varying_mu:
        ...
        outdict = dec_model.cluster_varying_mu(X, y_dec, roi_labels, classes, save_to, labeltype, update_interval=1) # 10 epochs# ~ per 1/3 of data 798/48=16 update twice per epoch ~ N/(batch size)=iterations to reach a full epochg
        ...
        dec_model_save= gzip.open(os.path.join(save_to,'iter1_dec_model_z{}_varying_mu{}_exp3_y{}.arg'.format(znum,num_centers,labeltype)), 'wb')
        ...
        fig2.savefig(save_to+os.sep+'iter1_DEC_z{}_varying_mu{} - unsuprv acc vs iteration.pdf'.format(znum,num_centers), bbox_inches='tight')    
        plt.close(fig2)
        ...
        zfinal.to_csv(save_to+os.sep+'iter1_zfinal_z{}_varying_mu{}.csv'.format(znum,num_centers), sep=',', encoding='utf-8', header=False, index=False)
        ...
        # save model saving params into a numpy array
        outdict_save= gzip.open(os.path.join(save_to,'iter1_outdict_z{}_varying_mu{}_exp3_y{}.arg'.format(znum,num_centers,labeltype)), 'wb')
    ...
    fig1.savefig(save_to+os.sep+'iter1_allDEC_varying_mu_Accu_NMI_z{} Dataset_unsuprv acc vs iteration.pdf'.format(znum), bbox_inches='tight')    
