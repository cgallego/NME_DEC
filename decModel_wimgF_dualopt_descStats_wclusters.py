...
class DECModel(model.MXModel):
    ...
    def cluster(self, X_train, y_dec_train, y_train, classes, batch_size, save_to, labeltype, update_interval, logger):
        ...
        self.argsd = {k: mx.nd.array(v.asnumpy(), ctx=self.xpu) for k, v in self.args.items()}
        ## embedded point zi 
        self.z = model.extract_feature(self.feature, self.argsd, None, test_iter, N, self.xpu).values()[0]        
        ...
        kmeans.fit(self.z)
        self.argsd['dec_mu'][:] = kmeans.cluster_centers_
        ...
        def ce(label, pred):
            ...
            overall_metric = -np.log(mean_auc) - 10*np.log(1-DECmetric)
            ...
            if(overall_metric <= self.maxAUC):
                ...
                for key, v in self.argsd.items():
                    self.best_args[key] = self.argsd[key]
                ...
                self.best_args['dec_mu'][:] = self.argsd['dec_mu'].asnumpy()
        ...
        def refresh(i): # i=3, a full epoch occurs every i=798/48
            if i%self.best_args['update_interval'] == 0:
                self.z = list(model.extract_feature(self.feature, self.argsd, None, test_iter, N, self.xpu).values())[0]
                self.p = np.zeros((self.z.shape[0], self.best_args['num_centers']))
                self.dec_op.forward([self.z, self.argsd['dec_mu'].asnumpy()], [self.p])
                self.best_args['dec_mu'] = self.argsd['dec_mu']
                ...
                if(i==0):
                    ...
                    self.Z_tsne = self.tsne.fit_transform(self.z)  
        ...
        solver.solve(self.xpu, self.loss, self.argsd, self.args_grad, None,
                     train_iter, 0, 1000000000, {}, False)
        self.end_args = self.argsd
        self.best_args['end_args'] = self.argsd
...
#    logger.addHandler(ch)
    ...
    latent_size = [input_size/rxf for rxf in [2]] # 25
