...
class DECModel(model.MXModel):
    ...
    def cluster(self, X_train, y_dec_train, y_train, classes, batch_size, save_to, labeltype, update_interval, logger):
        ...
        def ce(label, pred):
            ...
            print("DECmetric = {}".format( -np.log(1-DECmetric)))
            ...
            #y_pred_given_x = p_y_given_x.argmax(axis=1)
            ...
            overall_metric = -np.log(1-DECmetric-NLL) #NLL +10*DECmetric  # -1*(np.log(NLL) +np.log(DECmetric)) 
...
if __name__ == '__main__':
    ...
    latent_size = [input_size/rxf for rxf in [2,5,15]] #
